...
export default async (req, res) => {
  ...
  try {
    ...
    // Detect PayHero callback format
    const isPayHero = payload.status && payload.response && (payload.response.CheckoutRequestID || payload.response.ExternalReference);

    if (isPayHero) {
      console.log('Detected PayHero callback format');
      ...
      const isSuccess = ResultCode === 0 && payHeroStatus?.toLowerCase() === 'success';
      const newAttemptStatus = isSuccess ? 'success' : 'failed';
      ...
      if (checkoutId) {
        ...
        if (isSuccess) {
          const applicationsUpdateQuery = appSupabase
            .from('applications')
            .update({ payment_status: 'paid', payment_reference: checkoutId });
          ...
          const { error: applicationsUpdateError } = paymentAttempt?.application_id
            ...
            : await applicationsUpdateQuery.eq('payment_reference', checkoutId);
      ...
      return res.status(200).json({
        success: true,
        message: 'PayHero callback received',
        paymentStatus: newAttemptStatus
      });
    }
